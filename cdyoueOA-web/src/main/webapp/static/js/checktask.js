...
// 标签绑定点击事件
$('#checkTaskTab a').on('click', function () {
    var that = $(this);
    if (!that.parent().hasClass('active')) {
        $dg.bootstrapTable('refresh', {url: basePath + '/task/findCheckTask.do?status=' + that.data('tag')});
    }
});
// 查询
$('#ctsearch').on('click', function () {
    var query = $('#ct_title').val();
